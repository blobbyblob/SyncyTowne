...
local WARN_WARNING_STREAM = warn;
...
local ScopedHandlers = {
	userdata = function(ty, obj)
		return typeof(obj) == ty;
	end;
	userdata_Instance = function(ty, obj)
		return obj.ClassName == ty;
	end;
};
local ScopedDescriptions = {
	userdata = function(obj)
		return typeof(obj);
	end;
	userdata_Instance = function(obj)
		return obj.ClassName;
	end
};

--[[ @brief Emits an error if obj does not match objType.
	@param name The name to be used in describing the error in case of failure.
	@param objType The expected type. This is of the following form:
		type|type|...
		type represents a built-in Lua type (string, number), nil, or can be scoped to represent ROBLOX types, e.g.: userdata:Instance:TextLabel, userdata:Vector3, userdata:EnumItem:TextXAlignment
	@param obj The object to test.
--]]
function Log.AssertType(name, objType, obj)
	--Split objType along vertical bars. If it matches any of these, we're good to go.
	for scopedType in string.gmatch(objType, "[^|]+") do
		local scopes = {};
		for scope in string.gmatch(scopedType, "[^:]+") do
			table.insert(scopes, scope);
		end
		if scopes[1] == "nil" and obj == nil then
			return;
		elseif obj ~= nil then
			local fn = function(nextScope, obj) return type(obj) == nextScope; end
			local prefix;
			local i = 1;
			while fn and scopes[i] and fn(scopes[i], obj) do
				prefix = (prefix and prefix .. "_" or "") .. scopes[i]
				fn = ScopedHandlers[prefix];
				i = i + 1;
			end
			if not fn or not scopes[i] then
				return;
			end
		end
	end
	...
	if obj == nil then
		...
		typeDescription = type(obj);
		while ScopedDescriptions[typeDescription] do
			local n = ScopedDescriptions[typeDescription](obj);
			if n then
				typeDescription = typeDescription .. "_" .. n;
			else
				break;
			end
		end
	...
	Log.Error(2, "invalid type for %s; expected %s, got %s", name, objType, string.gsub(typeDescription, "_", ":"));
end
